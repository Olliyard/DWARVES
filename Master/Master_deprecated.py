...
class Master:
    def __init__(self, observationFrequency=0):
        ...
        self.ser = serial.Serial(USB_PORT, 9600, timeout=2)
    ...
    def setTemperature(self, updatedColony):
        ...
        if colonyInstance.startTime <= datetime.now().time() <= colonyInstance.dayInterval:    
            # Set day temperature
            self.ser.write(f'<sett,1,20>'.encode('utf-8'))
            print(self.ser.usb.readline().decode().strip())      # Decode and remove whitespace and print return message
            ...
            self.ser.write(f'sett,1,12'.encode('utf-8'))
            print(self.ser.readline().decode().strip())     # Decode and remove whitespace and print return message
    ...
    def setLight(self, updatedColony):
        ...
        if colonyInstance.startTime <= datetime.now().time() <= colonyInstance.dayInterval:    
            # Set day light
            self.ser.write(f'<setl,{colonyInstance.id},{colonyInstance.redDay},{colonyInstance.blueDay}>'.encode('utf-8'))
            print(self.ser.readline().decode().strip())      # Decode and remove whitespace and print return message
