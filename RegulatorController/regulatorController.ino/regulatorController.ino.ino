...
// Variables
int dutyCycle[3] = {0, 0, 0}; // 0 - red, 1 - blue, 2 - temp
int deviceCount = 0;
float tempC;
...
void setup() {
  ...
  struct Colony colonies[maxColonies];
}
...
void loop() {
  ...
  if (newData == true){
    ...
    currColony = parseData();
    // Go through PWM and set it for each channel.
    setPWM(currColony, colonies[currColony].redBrightness, colonies[currColony].blueBrightness, colonies[currColony].setTemp);
    newData = false;
  }
...
void setPWM(int colonyID, int red, int blue, int temp) {
  ...
  for (i=0; i<=maxColonies*3; i++){
    if (colonyID == 1) {
      digitalWrite(PIN_A, i); // 
      digitalWrite(PIN_B, i);
      digitalWrite(PIN_C, i);
